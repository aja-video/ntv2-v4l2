...
int ntv2_features_configure(struct ntv2_features *features, u32 id)
{
	...
	case NTV2_DEVICE_ID_KONAHDMI2RX:ntv2_features_konahdmi2rx(features);	break;
								  ...
								  int channel_index)
{
	...
	for (num = 0; num < NTV2_MAX_INPUT_CONFIGS; num++)
	{
		if (features->csc_config[channel_index][num] == NULL)
			break;
	}
									  ...
									  int channel_index)
{
	...
	return features->csc_config[channel_index][0];
}
...
ntv2_features_hdmi_edid_type(struct ntv2_features *features, int index)
{
	if ((features == NULL) ||
		(index < 0) ||
		(index >= sizeof(features->hdmi_edid)))
		return ntv2_edid_type_unknown;
	
	return features->hdmi_edid[index];
}
...
static void ntv2_features_initialize(void) {
	...
	/* hdmi inputs */
	nic = &nic_hdmi_adv_14_1;
	memset(nic, 0, sizeof(struct ntv2_input_config));
	nic->name = "HDMI 1";
	nic->type = ntv2_input_type_hdmi_adv;
	...
	nic = &nic_hdmi_aja_20_1;
	memset(nic, 0, sizeof(struct ntv2_input_config));
	nic->name = "HDMI 1";
	nic->type = ntv2_input_type_hdmi_aja;
...
#ifdef NTV2_RGB_PIXEL_FORMATS
	/* RGB32 format */
	npf = &npf_rgba;
	memset(npf, 0, sizeof(struct ntv2_pixel_format));
	npf->name = "XRGB32";
	npf->v4l2_pixel_format = V4L2_PIX_FMT_XRGB32;
	npf->ntv2_pixel_format = ntv2_kona_fbf_rgba;
	npf->pixel_flags =
		ntv2_kona_pixel_rgb |
	...
	memset(npf, 0, sizeof(struct ntv2_pixel_format));
	npf->name = "XBGR32";
	npf->v4l2_pixel_format = V4L2_PIX_FMT_XBGR32;
...
static void ntv2_features_corvidhbr(struct ntv2_features *features) 
{
	...
	features->input_config[0][0] = &nic_hdmi_adv_14_1;
...
static void ntv2_features_konahdmi2rx(struct ntv2_features *features) 
{
	...
	features->device_name = "Kona HDMI 2RX";
